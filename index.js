...
let main = () => {
  ...
  const sendMessage = (message) => {
    ...
    var encodedMessageStr = Buffer.from(messageStr, "utf-8").toString();

    bufferHeader.writeUInt32LE(encodedMessageStr.length, 0);

    let bufferMessageContent = Buffer.from(encodedMessageStr);

    let finalMessage = Buffer.concat([bufferHeader, bufferMessageContent]);
